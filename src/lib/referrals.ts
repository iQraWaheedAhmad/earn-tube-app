...
export async function processReferralReward(
  ...
  try {
    ...
    console.log("=== CREATING COMPLETED REFERRAL REWARD ===", {
      referrerId,
      referredId: userId,
      rewardAmount,
    ...
    // Create referral record with COMPLETED status and add to balance in a transaction
    const result = await prisma.$transaction(async (tx) => {
      ...
      console.log("=== CURRENT REFERRER BALANCE ===", {
        ...
        rewardAmount,
        newBalance: (currentReferrer?.balance || 0) + rewardAmount,
      });
      ...
      return referralUnknown;
    });

    const referral = result as {
      id: string;
      rewardAmount: number;
      status: string;
    };

    console.log("=== REFERRAL REWARD CREATED AND COMPLETED ===", {
